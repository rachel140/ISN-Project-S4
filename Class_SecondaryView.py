...
class SecondaryView:
    ...
    def top_or_side(self):
        # Returns the current view mode ('top' or 'side') from controller
    ...
    def generate_base_image(self, base_width, base_height, year, scenario):
        ...
        # Save lat/lon arrays to instance variables for potential future use
        self.lats = lats
        self.lons = lons

        # reverse the lattitude to start from north and space evenly coordinates
        ...
        #-------------------------original code-------------------------------#
        # elev_sub = np.zeros((base_height, base_width)) #like initialising dico
        # for i in range(base_height):
        #     for j in range(base_width):
        #         elev_sub[i, j] = elevs[lat_indices[i], lon_indices[j]]

        #------------------------improved by AI-------------------------------#
        elev_sub = elevs[lat_indices[:, None], lon_indices[None, :]]
    ...
    def redraw(self):
        ...
        if not hasattr(self, "photo") or self.photo is None:
            print("[DEBUG] No photo image available to draw")
            return

        if not hasattr(self, "canvas") or self.canvas is None:
            print("[DEBUG] Canvas is not initialized")
            return
        
        # Clear previous drawings on canvas
        self.canvas.delete("all")
        self.base_image = self.canvas.create_image(int(self.pan_x), int(self.pan_y), image=self.photo, anchor=ctk.NW)
        ...
        # resize the baseimage using nearest neighbour method (=fastest but blocky, use BICUBIC for best quality)
        base_image = self.base_image.resize((new_w, new_h), Image.NEAREST)
...
#----------------------------Created with help of AI--------------------------#
    ...
    def create_map(self, frame, width, height, year, scenario):
        ...
        canvas = ctk.CTkCanvas(frame, width=width, height=height, bg="white")
        canvas.pack(fill=ctk.BOTH, expand=True)
        self.canvas = canvas # Keep a reference to the canvas (used in profile view)
        ...
        pil_image = self.generate_base_image(width, height, year, scenario)
        # Convert it to a PhotoImage
        self.photo = ImageTk.PhotoImage(pil_image)
        # Initial drawing of the map on the canvas
        self.canvas.after_idle(self.safe_redraw)
        
    def safe_redraw(self):
        try:
            self.redraw()
        except Exception as e:
            print(f"[ERROR] Redraw failed: {e}")
